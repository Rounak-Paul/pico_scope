...
class MUSIC:
    ...
    def play_note(self,Note, Duration):
        ...
        if Note == "S":
            ...
            self.speaker.duty_u16(0)
            sleep(0.05)
            self.speaker.duty_u16(1500)
        
            self.speaker.freq(MusicNotes[Note])
            sleep(Duration)
    
    def play_mario(self):
        for note_c in mario:
            self.play_note(note_c, 0.13)
    
    def play_digital(self,digital_val,Duration):
        if digital_val == 0:
            sleep(Duration)
        if digital_val == -1:
            self.speaker.duty_u16(0)
